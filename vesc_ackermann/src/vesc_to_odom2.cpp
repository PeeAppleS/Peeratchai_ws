...
VescToOdom::VescToOdom(const rclcpp::NodeOptions & options)
    : Node("vesc_to_odom_node", options),
      odom_frame_("odom"), // Keep "odom" frame
      map_frame_("map"),   // Add "map" frame
...
void VescToOdom::updateCallback()
{
  ...
  Odometry odom;
  odom.header.frame_id = odom_frame_; // Frame of odometry
  ...
  if (publish_tf_) {
    ...
    TransformStamped odom_to_base_tf;
    odom_to_base_tf.header.frame_id = odom_frame_;
    odom_to_base_tf.child_frame_id = base_frame_;
    odom_to_base_tf.header.stamp = now();
    odom_to_base_tf.transform.translation.x = x_;
    odom_to_base_tf.transform.translation.y = y_;
    odom_to_base_tf.transform.translation.z = 0.0;
    odom_to_base_tf.transform.rotation = odom.pose.pose.orientation;

    // Publish transform from map to odom
    TransformStamped map_to_odom_tf;
    map_to_odom_tf.header.frame_id = map_frame_;
    map_to_odom_tf.child_frame_id = odom_frame_;
    map_to_odom_tf.header.stamp = now();
    map_to_odom_tf.transform.translation.x = 0.0; // Set to 0 or actual map-to-odom offset
    map_to_odom_tf.transform.translation.y = 0.0; // Set to 0 or actual map-to-odom offset
    map_to_odom_tf.transform.translation.z = 0.0;
    map_to_odom_tf.transform.rotation.w = 1.0; // Assuming no rotation
    map_to_odom_tf.transform.rotation.x = 0.0;
    map_to_odom_tf.transform.rotation.y = 0.0;
    map_to_odom_tf.transform.rotation.z = 0.0;

    if (rclcpp::ok()) {
      tf_pub_->sendTransform(odom_to_base_tf);
      tf_pub_->sendTransform(map_to_odom_tf);
    }
  }
